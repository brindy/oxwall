...
class BASE_CTRL_ComponentPanel extends OW_ActionController
{
    ...
    private function action( $place, $userId, $customizeMode, $customizeRouts, $componentTemplate, $responderController = null )
    {
        ...
        if ( !$this->componentAdminService->isCacheExists($place) )
        {
            $state = array();
            ...
            $state['defaultScheme'] = $this->componentAdminService->findSchemeByPlace($place);
        ...
        {
            if ( !$this->componentEntityService->isEntityCacheExists($place, $userId) )
            {
                $userCache = array();
    ...
    public function dashboard( $paramList )
    {
        ...
        $dashboardMenu = new BASE_CMP_DashboardContentMenu();
        $dashboardMenu->getElement('widget_panel')->setActive(true);
        ...
        if ( BOL_AuthorizationService::getInstance()->isModerator() || OW::getUser()->isAdmin() )
        {
            ...
            {
                $flags[] = array('type' => $type['type'], 'langKey' => $type['langKey'], 'count' => BOL_FlagService::getInstance()->countFlaggedItems($type['type']));
            }

            $this->assign('flags', $flags);
        }

        $this->addComponent('contentMenu', $dashboardMenu);

        $this->setDocumentKey('base.user_dashboard');
    }
    ...
    public function myProfile( $paramList )
    {
        ...
        $this->addComponent('profileActionToolbar', new BASE_CMP_ProfileActionToolbar($userId));
    ...
    public function profile( $paramList )
    {
        ...
        $this->addComponent('profileActionToolbar', new BASE_CMP_ProfileActionToolbar($userDto->id));
        ...
        $this->setDocumentKey('base.profile_page');
    }
    ...
    public function privacyMyProfileNoPermission( $params )
    {
        ...
        $this->addComponent('profileActionToolbar', new BASE_CMP_ProfileActionToolbar($user->id));
    ...
    public function index( $paramList )
    {
        ...
        $this->setDocumentKey('base.index_page');
    }
