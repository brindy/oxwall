...
class BASE_Export extends DATAEXPORTER_CLASS_Export
{
    ...
    private function exportConfigs( ZipArchive $za, $archiveDir )
    {
        ...
        $query = " SELECT `key`, `name`, `value`, `description` FROM " . BOL_ConfigDao::getInstance()->getTableName() . " WHERE name NOT IN ( 'maintenance', 'soft_build', 'update_soft', 'site_installed' )
    ...
    private function exportThemes( ZipArchive $za, $archiveDir )
    {
        ...
        $this->zipFolder($za, $currentThemeUserfilesDir, $archiveDir . '/themes/');
    }
