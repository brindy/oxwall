...
class BOL_AuthorizationUserRoleDao extends OW_BaseDao
{
    ...
    public function getRoleListOfUsers( array $users )
    {
        if ( count($users) < 1 )
        {
            return $users;
        ...
        $query = "SELECT `userId`,`name`,`custom` FROM {$this->getTableName()} ur 
                  LEFT JOIN {$roleTable} r ON ur.roleId=r.id
                  WHERE ur.userId IN({$this->dbo->mergeInClause($users)}) AND r.displayLabel=1 GROUP BY ur.userId";
