...
class INSTALL_CTRL_Install extends INSTALL_ActionController
{
    ...
    public function install()
    {
        ...
        if ( OW::getRequest()->isPost() )
        {
            if ( !empty($errorDirs) )
            {
                INSTALL::getFeedback()->errorMessage('Some derictories are not writable');
            ...
            {
                INSTALL::getFeedback()->errorMessage('SQL import failed');
    ...
    private static function sqlImport( $sqlFile )
    {
        ...
        while ( false !== ($line = fgets($fd, 10240)) )
        {
            ...
            try {
                ...
                throw new LogicException('SQL import failed');
            }
