...
class OEmbedDefaultProvider extends OEmbedProvider
{
    ...
    private function parsePage( $url )
    {
        ...
        $title = empty($matches[1]) ? null : $matches[1];
        ...
        $description = empty($matches[1]) ? null : $matches[1];

        $matches = array();
        preg_match('/<\s*img\s*.*?src=[\'"](.+?)[\'"].*?>/i',$content,$matches);
        $img = empty($matches[1]) ? null : $matches[1];
