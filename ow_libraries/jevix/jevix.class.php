        ...
        while ( ($this->curCh != '<') && $this->curChClass )
        {
            ...
            elseif ( $this->isAutoLinkMode && ($this->curChClass & self::LAT) && $this->openedTag != 'a' && $this->url($url, $href) )
            {
                ...
                if ( strlen($linkLabel) > 60 )
                {
                    ...
                    $domain = (!empty($arr['scheme']) ? $arr['scheme'] . '://' : '' ) . $arr['host'] . (!empty($arr['port']) ? ':' . $arr['port'] : '' ) . '/';
                ...
                $text.= $this->makeTag('a', array('href' => $href, 'class' => 'ow_autolink', 'target' => '_blank'), $linkLabel, false);
                $newWord = true;
            }
