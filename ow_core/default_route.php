...
class OW_DefaultRoute
{
    ...
    public function generateUri( $controller, $action = null, array $params = array() )
    {
        ...
        return strtolower($plugin->getModuleName()) . '/' . substr(UTIL_String::capsToDelimiter($className, '-'), 1) . '/' . UTIL_String::capsToDelimiter($action, '-') . '/' . $paramString;
    }
    ...
    {//TODO check if method is in try/catch
        ...
        for ( $i = 0; $i < $arraySize; $i++ )
        {
            if ( $i === 0 )
            {
                try
                {
                    $classPrefix = strtoupper(OW::getPluginManager()->getPluginKey($uriArray[$i])) . '_CTRL';
                }
            ...
            if ( $i === 1 )
            {
                ...
                $dispatchAttrs['controller'] = $classPrefix . '_' . UTIL_String::delimiterToCaps('-' . $uriArray[$i], '-');
                continue;
            }
