...
class Form
{
     ...
     * @return string
     */
    public function render( $formContent, $decorator = null )
    {
        $formElementJS = '';
        ...
        foreach ( $this->elements as $element )
        {
            ...
            $formElementJS .= "window.owForms['" . $this->getName() . "'].addElement(formElement);" . PHP_EOL;
        }

        $jsString = "if( !window.owForms ){window.owForms = {};}

			var form;
			form = new OwForm(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");
            form.reset = " . json_encode($this->getAjaxResetOnSuccess()) . ";
            form.ajax = " . json_encode($this->isAjax()) . ";
            form.ajaxDataType = " . json_encode($this->getAjaxDataType()) . ";
            form.validateErrorMessage = " . json_encode($this->emptyElementsErrorMessage) . ";
        ...
        return UTIL_HtmlTag::generateTag('form', $this->attributes, true, PHP_EOL . $hiddenFieldString . $formContent . PHP_EOL);
    }
}
